...
main() {
  RELEASES_URL=https://api.github.com/repos/shahdloo/resume/releases/latest
  ...
  git add .
  git commit -m "Update cv to version ${VERSION}"
  ...
  if [[ "${BRANCH}" == "${PULL_REQUEST_BRANCH}" ]]; then
    ...
    create_pull_request "${BRANCH}" "${PULL_REQUEST_BRANCH}" "${PULL_REQUEST_BODY}" "Update cv to ${VERSION}"
  fi
}
